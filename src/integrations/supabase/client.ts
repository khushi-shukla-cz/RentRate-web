...
// Prevent app boot failure in local fallback mode when .env values are missing.
const fallbackUrl = 'http://127.0.0.1:54321';
const fallbackKey = 'public-anon-key';

if (!HAS_SUPABASE_CONFIG && import.meta.env.DEV) {
  console.warn(
    '[Supabase] Missing VITE_SUPABASE_URL or VITE_SUPABASE_PUBLISHABLE_KEY. Using fallback client for local mode.'
  );
}
